...
I made sure this container will run on an Intel/AMD CPU or ARM64 CPU by default (CPU-Only container). However I built the GPU container to use an NVIDIA GPU which is installed on the system whether with an Intel/AMD CPU or an ARM64 CPU.
Note that if the GPU 'fails' to be seen, the GPU container will default to use the main cores.
